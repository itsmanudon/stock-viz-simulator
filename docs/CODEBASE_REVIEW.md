...
> (`news_articles.url` is unique); §2.3 was worse than described
> (`RATELIMIT_ENABLED=0` never disabled the limiter at all); and the review
> missed that `alembic upgrade head` was outright broken on two heads — which
> the API Dockerfile runs on every boot.
...
**Worse than written.** Found while fixing this: `RATELIMIT_ENABLED=0` never
disabled the limiter. slowapi reads that variable itself and keeps the raw
string, so `limiter.enabled` was `"0"` — truthy. The switch documented in
`CLAUDE.md` and `.env.example` as the way to turn limiting off for tests and
load scripts had never worked.
...
- **No `alembic check`** — nothing catches model/migration drift, and there are
  ...
  **This was hiding a live break:** `dev` had *two* alembic heads, so
  `alembic upgrade head` failed outright — and `apps/api/Dockerfile` runs
  exactly that on boot. Six enum columns had drifted from the models too.
