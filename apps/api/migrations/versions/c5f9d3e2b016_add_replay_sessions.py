...
Isolated ReplaySession state for SIM-05. Replay fills do not FK onto
``trades`` and are not backfilled from live paper.
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("clock_now", sa.DateTime(), nullable=False),
    ...
    op.create_index(
        op.f("ix_replay_sessions_clock_now"), "replay_sessions", ["clock_now"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index(op.f("ix_replay_sessions_clock_now"), table_name="replay_sessions")
