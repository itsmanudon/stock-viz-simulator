#!/usr/bin/env bash
# Boots Postgres + Adminer via Docker Compose, waits for the DB to be ready,
# then applies Alembic migrations and seeds initial data.
# Run from the repo root: ./scripts/db-init.sh

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"

# ── Step 1: start containers ────────────────────────────────────────────────
echo "==> Starting Postgres + Adminer..."
docker compose -f infra/docker-compose.yml up -d

# ── Step 2: wait for Postgres to accept connections ─────────────────────────
echo "==> Waiting for Postgres on 127.0.0.1:5434..."
until docker compose -f infra/docker-compose.yml exec -T postgres \
    pg_isready -U stockviz -d stockviz_dev -q 2>/dev/null; do
  sleep 1
done
echo "    Postgres is ready."

# ── Step 3: apply migrations ─────────────────────────────────────────────────
echo "==> Applying Alembic migrations..."
uv --directory apps/api run alembic upgrade head

# ── Step 4: seed and backfill ────────────────────────────────────────────────
echo "==> Seeding symbol list..."
uv --directory apps/api run python -m stockviz.cli seed

echo "==> Running one-time CSV backfill..."
uv --directory apps/api run python -m stockviz.cli backfill

echo ""
echo "Done. Services:"
echo "  Postgres  127.0.0.1:5434"
echo "  Adminer   http://localhost:8080  (server: postgres, user/pass/db: stockviz / stockviz_dev / stockviz)"
echo ""
echo "Next: open two terminals and run:"
echo "  pnpm api:dev   # FastAPI on http://127.0.0.1:8000"
echo "  pnpm dev:web   # Next.js on http://localhost:3000"
